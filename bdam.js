...
placeObject = function(event) {
  ...
  if(event.target.getAttribute("color") == colors[0]){
    if(counter1 == 0) {
      scene.appendChild(makeBox("test", document.getElementById("js--camera-box").getAttribute("color"),"-2 0.626 -6", "1"));
      console.log("if");
    }
    counter1++;
  } else if(event.target.getAttribute("color") == colors[1]){
    if(counter2 == 0) {
      scene.appendChild(makeBox("test", document.getElementById("js--camera-box").getAttribute("color"),"0 0.626 -6", "1"));
    } else if(counter2 == 1) {
      scene.appendChild(makeBox("test", document.getElementById("js--camera-box").getAttribute("color"),"0 1.626 -6", "1"));
    }
    counter2++;
    ...
    if(counter3 == 0) {
      ...
      scene.appendChild(makeBox("test", document.getElementById("js--camera-box").getAttribute("color"),"2 1.626 -6", "1"));
    } else if(counter3 == 2) {
      scene.appendChild(makeBox("test", document.getElementById("js--camera-box").getAttribute("color"),"2 2.626 -6", "1"));
    }
    counter3++;
  }
